...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const fileName =
        `${crypto.randomUUID()}.${normalizedExtension}`;
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (statusError) {
        console.error(
          "Inspection status update error:",
          statusError
        );
  ...
  async (req, res) => {
    try {
      ...
      if (!findingId || !verificationStatus) {
        return res.status(400).json({
          error:
            "findingId and verificationStatus are required"
        });
      }
      ...
      if (inspection.officer_id !== officer.id) {
        return res.status(403).json({
          error:
            "You are not authorized to verify this inspection"
        });
      }
